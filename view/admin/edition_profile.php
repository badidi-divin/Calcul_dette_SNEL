<?php
    ...
    $requser=$connexion->prepare("SELECT * FROM user WHERE id=?");
    $requser->execute(array($_SESSION['id']));
    $user=$requser->fetch();
    if (isset($_POST['pseudo']) AND !empty($_POST['pseudo']) AND $_POST['pseudo'] !=$user['username']) {
        $pseudo=htmlspecialchars($_POST['pseudo']);
        $insertpseudo=$connexion->prepare('UPDATE user SET username=? WHERE id=?');
    ...
    if (isset($_POST['password']) AND !empty($_POST['password']) AND $_POST['password'] !=$user['password']) {
        ...
        $insertpassword=$connexion->prepare('UPDATE user SET password=? WHERE id=?');
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                            ...
                                            <form action="" method="post" role="search" class="sr-input-func">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="pseudo" class="form-control" value="<?= $user['username']; ?>">
                                                </div>
                                                 <div class="form-group">
                                                     <label class="control-label">
                                                        Password
